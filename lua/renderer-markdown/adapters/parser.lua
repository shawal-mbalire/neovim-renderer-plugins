...
function markdown_parser.parse(markdown)
	...
	while line_index <= #lines do
		...
		elseif
			line:match("|")
			and line_index + 1 <= #lines
			and lines[line_index + 1]:match("^|?[%s:]+%-")
		then
...
function markdown_parser.parse_blockquote(lines, start)
	...
	return {
		node = markdown_models.create_node("blockquote", {
			children = {
				markdown_models.create_node("text", { content = table.concat(content_lines, "\n") }),
			},
		}),
		next_line = i,
	}
...
function markdown_parser.parse_task_list(lines, start)
	...
	while i <= #lines do
		...
		if done_match then
			table.insert(
				items,
				markdown_models.create_node("task_item", {
					checked = true,
...
function markdown_parser.parse_list(lines, start, ordered)
	...
	while i <= #lines do
		...
		if content then
			table.insert(
				items,
				markdown_models.create_node("list_item", {
	...
	return {
		node = markdown_models.create_node(
			ordered and "ordered_list" or "unordered_list",
			{ children = items }
		),
		next_line = i,
	}
...
function markdown_parser.parse_table(lines, start)
	...
	table.insert(
		rows,
		markdown_models.create_node("table_row", {
...
function markdown_parser.parse_paragraph(lines, start)
	...
	while
		i <= #lines
		and lines[i]:match("^%s*$") == nil
		and not markdown_parser.is_block_start(lines[i])
	do
